...
void init_colours(void) {
    ...
    for (int i = 0; i < max_iter; i++){
        char r = sinf(freq * i + 1) * 127 + 128;
        char g = sinf(freq * i + 3) * 127 + 128;
        char b = sinf(freq * i + 5) * 127 + 128;
...
/* Julia set for the recursion:
      z_{n+1} = z_n^3 + C
   For each complex number C = cr + i*ci (coming from a pixel),
   start with z_0 = 0 and iterate.  If |z_n|^2 > 16 before max_iter
   iterations, we say it escapes; otherwise we treat it as in the set.
*/
__device__ uint32_t julia_double(double cr, double ci, int max_iter) {
    ...
    for (i = 0; i < (uint32_t)max_iter; i++){
        ...
        double new_zr = zr2 * zr - 3.0 * zr * zi2 + cr;
        double new_zi = 3.0 * zr2 * zi - zi2 * zi + ci;
...
static void display_double(double xcen, double ycen, double scale,
    ...
    (void)numBlocks; // not used, but kept to match original structure
    ...
    julia_kernel<<<n, m>>>(dev_counts, xmin , ymin, step,
                           max_iter, dim, colors);
...
int main(int argc, char** argv){
    ...
    double xcen  = 0.0;
    double ycen  = 0.156;  // same as original; feel free to change
    double scale = 3.0;
